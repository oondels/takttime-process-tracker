...
```json
{
  ...
  "tech": {
    "amqp_host": "amqp://user:password@broker-host/",
    "amqp_user": "",
    "amqp_pass": "",
...
AMQP_URL = tech_config.get("amqp_host") or os.getenv("AMQP_URL", "amqp://user:password@broker-host/")
...
MQTT_BROKER = tech_config.get("mqtt_broker") or os.getenv("MQTT_BROKER", "broker-host-or-ip")
MQTT_PORT = tech_config.get("mqtt_port", 1883)
MQTT_USER = tech_config.get("mqtt_user") or os.getenv("MQTT_USER", "dass")
MQTT_PASS = tech_config.get("mqtt_pass") or os.getenv("MQTT_PASS", "mqtt-password")
DEVICE_ID = device_config.get("cell_number") or os.getenv("DEVICE_ID", "cost-2-2408")
...
```python
            if not mqtt_broker:
                ...
                load_dotenv()
                mqtt_broker = os.getenv("MQTT_BROKER", "broker-host-or-ip")
                mqtt_user = os.getenv("MQTT_USER", "dass")
                mqtt_pass = os.getenv("MQTT_PASS", "mqtt-password")
...
```python
        ...
        self.mqtt_broker_input.setPlaceholderText("broker-host-or-ip")
...
async def test():
    async with aiomqtt.Client('broker-host-or-ip', 1883, username='dass', password='mqtt-password') as client:
...
sudo rabbitmqctl add_user dass mqtt-password
